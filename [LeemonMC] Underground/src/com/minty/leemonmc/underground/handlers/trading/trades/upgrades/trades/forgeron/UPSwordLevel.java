...
public class UPSwordLevel extends Trade {

	int level;
	int price;
	
	public UPSwordLevel(TradeCategory category, int _level, int _price)
	{
		super(category);
		price = _price;
		level = _level;
	...
	{
		return "§6Amélioration Épée Niveau " + level;
	}
	
	@Override
	public String getDescription()
	{
		return "§7Améliore votre §eépée §7au §eniveau " + level + "§7.";
	}
	...
	public boolean isPurchased(Player player)
	{
		...
		return data.getSwordLevel() >= level;
	}
	
	@Override
	public ItemStack getIcon(Player player)
	{
		return StuffHandler.getSword(level);
	}
	...
	{
		UndergroundData data = Underground.getInstance().getUndergroundDataHandler().getPlayerData(player);
		player.sendMessage("§6§lUnderground §f» §7Votre §6épée §7a été améliorée au §6niveau " + level + " §7!");
		data.setSwordLevel(level);
